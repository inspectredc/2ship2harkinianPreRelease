...
extern DebugCamTextBufferEntry sDebugCamTextBuffer[22];

extern s16 sDebugCamTextEntryCount;

extern Color_RGBA8 sDebugCamTextColors[];
...
extern DebugDispObject* DebugDisplay_AddObject(f32 posX, f32 posY, f32 posZ, s16 rotX, s16 rotY, s16 rotZ, f32 scaleX,
...
extern void DebugCamera_DrawScreenText(GfxPrint* printer);

// #endregion

// #region z_debug_camera
...
s32 func_800B4088(DebugCam* debugCam, Camera* cam) {
    ...
    if (debugCam->sub.mode != 1) {
        ...
        func_800B3F94(&cam->focalActorPosRot, &debugCam->at, &lookAt->pos);
        func_800B3F94(&cam->focalActorPosRot, &debugCam->eye, &position->pos);
    }
...
s16 func_800B41DC(DebugCam* debugCam, s16 idx, Camera* cameraPtr) {
    ...
    if (debugCam->sub.mode != 1) {
        ...
        func_800B404C(&cameraPtr->focalActorPosRot, &lookAt->pos, &debugCam->at);
        func_800B404C(&cameraPtr->focalActorPosRot, &position->pos, &debugCam->eye);
    }
...
s32 func_800B42C0(DebugCam* debugCam, Camera* cameraPtr) {
    ...
    if (debugCam->sub.mode != 1) {
        ...
        func_800B3F94(&cameraPtr->focalActorPosRot, &debugCam->at, &lookAt->pos);
        func_800B3F94(&cameraPtr->focalActorPosRot, &debugCam->eye, &position->pos);
    }
...
s32 func_800B4370(DebugCam* debugCam, s16 idx, Camera* cam) {
    ...
    if (debugCam->sub.mode != 1) {
        ...
        if (debugCam->sub.unk_0C) {
            func_800B404C(&cam->focalActorPosRot, &position->pos, &at);
        } else {
            func_800B404C(&cam->focalActorPosRot, &lookAt->pos, &at);
        }
        debugCam->at = at;
    }
    geo.pitch = 0x2000;
...
void func_800B44E0(DebugCam* debugCam, Camera* cam) {
    ...
    if (!func_800BB2B4(&sDebugCamAnim.positionPos, &sDebugCamAnim.roll, &sDebugCamAnim.fov, debugCam->sub.position,
        ...
        if (debugCam->sub.mode != 1) {
            ...
            func_800B3FF4(&cam->focalActorPosRot, &sDebugCamAnim.lookAtPos, &debugCam->at);
            func_800B3FF4(&cam->focalActorPosRot, &sDebugCamAnim.positionPos, &debugCam->eye);
        }
...
void DebugCamera_Update(DebugCam* debugCam, Camera* cam) {
    ...
    *sp90 = temp_s6->pos;
    ...
    if (CHECK_BTN_ALL(sPlay->state.input[DEBUG_CAM_CONTROLLER_PORT].press.button, BTN_Z)) {
        ...
        sDebugCamAnim.unk_0A = 0;
        Audio_PlaySfx(NA_SE_SY_LOCK_ON);
            ...
            *sp80 = cam->eye;
            ...
            if (CHECK_BTN_ALL(sPlay->state.input[DEBUG_CAM_CONTROLLER_PORT].press.button, BTN_R) &&
                ...
                if (debugCam->sub.unkIdx == 0x80) {
                    ...
                    Audio_PlaySfx(NA_SE_IT_SWORD_PUTAWAY);
    ...
    if (debugCam->unk_00 == 1) {
        ...
        if (CHECK_BTN_ALL(sPlay->state.input[DEBUG_CAM_CONTROLLER_PORT].cur.button, BTN_CRIGHT) &&
            ...
            if (debugCam->sub.mode != 1) {
                ...
                spFC.yaw -= cam->focalActorPosRot.rot.y;
                DebugCamera_ScreenTextColored(
                    3, 22,
                    ((debugCam->sub.unk_08 == 1) && (debugCam->sub.unk_0A == 4) && !D_80161144) ? DEBUG_CAM_TEXT_GREEN
                    : !D_80161144                                                               ? DEBUG_CAM_TEXT_GOLD
...
char DebugCamera_InitCut(s32 idx, DebugCamSub* sub) {
    ...
    sDebugCamCuts[idx].lookAt = ZeldaArena_Malloc(i);
    ...
    sDebugCamCuts[idx].position = ZeldaArena_Malloc(i);
    if (sDebugCamCuts[idx].position == NULL) {
        // "Debug camera memory allocation failure"
        osSyncPrintf("%s: %d: デバッグカメラ メモリ確保失敗！！\n", "../db_camera.c", 2757);
        ZeldaArena_Free(sDebugCamCuts[idx].lookAt);
...
void DebugCamera_ResetCut(s32 idx, s32 shouldFree) {
    ...
    if (shouldFree) {
        ZeldaArena_Free(sDebugCamCuts[idx].lookAt);
        ZeldaArena_Free(sDebugCamCuts[idx].position);
    }
...
s32 DebugCamera_LoadCallback(char* c) {
    ...
    for (i = 0; i < ARRAY_COUNT(sDebugCamCuts) - 1; i++) {
        if (sDebugCamCuts[i].letter != '?') {
            ...
            sDebugCamCuts[i].lookAt = ZeldaArena_Malloc(ALIGN32(size));
            ...
            sDebugCamCuts[i].position = ZeldaArena_Malloc(ALIGN32(size));
...
s32 DebugCamera_SaveCallback(char* c) {
    ...
    if ((u32)sAllocSize < (freeSize + ret)) {
        if (!Mempak_Alloc(DEBUG_CAM_CONTROLLER_PORT, c, sAllocSize)) {
            return false;
        }
            ...
            switch (debugCam->sub.demoCtrlMenu) {
                ...
                default: {
                    if (Mempak_Init(DEBUG_CAM_CONTROLLER_PORT)) {
                        sMempakFiles = Mempak_FindFile(DEBUG_CAM_CONTROLLER_PORT, 'A', 'E');
