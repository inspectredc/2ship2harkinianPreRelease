...
extern OSMesgQueue* PadMgr_AcquireSerialEventQueue(void);
extern void PadMgr_ReleaseSerialEventQueue(OSMesgQueue* serialEventQueue);
...
s32 Mempak_Init(s32 controllerNb) {
    ...
    mq = PadMgr_AcquireSerialEventQueue();
    ...
    PadMgr_ReleaseSerialEventQueue(&gPadMgr);
...
s32 Mempak_GetFileSize(s32 controllerNb, char idx) {
    OSMesgQueue* mq = PadMgr_AcquireSerialEventQueue();
    OSPfsState state;
